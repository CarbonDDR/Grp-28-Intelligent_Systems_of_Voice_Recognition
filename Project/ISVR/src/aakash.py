...
def folder_finder(speak):
    ...
    if len(found_paths) == 0:
        ...
        for line in found_paths:
            ...
            j += 1
        print(
            "There are multiple folders with this name. Please specify the id of the path you want to open."
        )
        number = int(ask())
...
def Linux(file_name):
    cmd = subprocess.Popen(
        f"find . -name {file_name}", shell=True, stdout=subprocess.PIPE
    )
...
def Windows(file_name):
    cmd = subprocess.Popen(
        f"where /r \ {file_name}", shell=True, stdout=subprocess.PIPE
    )
    subprocess_return = cmd.communicate()
    ans = subprocess_return[0].decode("utf-8")
    paths = []
...
def ok():
    ...
    cons = [
        inp_unit in lengths and out_unit in lengths,
